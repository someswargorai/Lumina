...
import VideoCallNotification from "./components/video-call-notification";
...
const ChatPage = () => {
    ...
    //scenario i have made the call with my peerid
    ...
    // Call handling logic is managed in the main Peer useEffect at the top

    // this is the CALLER side (outgoing call), not receiver side.

    // peer.call() ALWAYS means:
    //“I am initiating a call to someone”

    // A starts call
    // socket.emit("call", { peerId: A })
    // B receives notification
    // socket.on("call", (data))

    // 👉 B is now only informed

    // BUT WebRTC connection has NOT started yet

    // B accepts call (THIS is where role flips)
    // peer.call(A.peerId, stream)

    // 👉 Now B becomes the caller in WebRTC terms
    const UserAcceptCall = async () => {

        // This is YOUR microphone (voice call only).
        const stream = await navigator.mediaDevices.getUserMedia({
            audio: true,
            video: false,
        });

        // You start a call to the other user
        const call = peerRef.current?.call(callData.peerId, stream);


        // I will now receive the other person’s audio stream
        call?.on("stream", (remoteStream) => {
            const audio = document.getElementById("remoteAudioStream") as HTMLAudioElement;
            if (audio) {
                audio.srcObject = remoteStream;
        ...
        <>
            <TooltipProvider delayDuration={300}>
                <div className="w-full h-full min-h-[calc(100vh-4rem)] flex items-center justify-center bg-[#f4f4f8] dark:bg-[#09090b] p-4 font-sans">
                    ...
                    <div className="relative w-full max-w-4xl h-[calc(100vh-6rem)] flex flex-col rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-xl shadow-zinc-200/60 dark:shadow-black/50">
                        ...
                        <div className="flex items-center justify-between px-5 py-3.5 border-b border-zinc-100 dark:border-zinc-800/80 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md z-10">
                            ...
                            <div className="flex items-center gap-1">
                                        ...
                                        <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-zinc-500 hover:text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-500/10"
                                            onClick={async () => {
                                                socketRef.current?.emit("call", {
                                                    roomId: `room_${[id, session?.id].sort().join("_")}`,
                                                    senderId: session?.id,
                                                    receiverId: id,
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-zinc-500 hover:text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-500/10" onClick={() => {
                    ...
                    <audio id="remoteAudioStream"></audio>
                </div>
            </TooltipProvider>

            {callTrue && (
                <CallNotification callData={callData} setCallTrue={setCallTrue} acceptCall={UserAcceptCall} rejectCall={rejectCall} />
            )}
