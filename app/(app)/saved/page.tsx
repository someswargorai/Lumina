...
export default function Saved() {
    ...
    if (isPageLoading) {
        return (
            <div className="flex flex-col gap-8 py-8 px-5 md:px-20">
                {[1, 2, 3].map((i) => (
    ...
    if (isError) {
        return (
            <div className="py-20 text-center">
                <p className="text-slate-500">Failed to load saved blogs. Please try again later.</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col px-5 md:px-20">
            <header className="py-8 border-b border-slate-100 mb-4 flex flex-col gap-2">
                ...
                <h1 className="text-3xl font-black text-slate-900 tracking-tight">Saved Stories</h1>
                <p className="text-slate-500 mt-1">Quickly access the articles and stories you&apos;ve saved for later.</p>
            </header>
                ...
                {posts.length === 0 ? (
                    <div className="col-span-12 py-24 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/50">
                        <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-6">
                            <Bookmark size={28} className="text-slate-200" />
                        </div>
                        <h3 className="text-lg font-bold text-slate-900 mb-2">No saved stories yet</h3>
                    ...
                    posts.map((post) => (
                        <div key={post.id} className="col-span-12">
                            <PostCard post={post} bookmarked={true} />
                        </div>
                    ))
                )}
            </div>
